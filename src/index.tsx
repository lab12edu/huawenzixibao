...
import { runSttAnchor, SttError, type SttWord } from './server/sttAnchor'
...
app.post('/api/oral/audit', apiLimiter, async (c) => {
  ...
  try {
    ...
    // Use SttError.code for specific, user-friendly messages.
    const code    = sttErr instanceof SttError ? sttErr.code : 'UNKNOWN'
    const message = sttErr?.message ?? String(sttErr)
    ...
    if (code === 'API_KEY_ERROR') {
      return c.json({
        error: 'API Key Error: Please check Google Cloud Billing / Permissions (Speech-to-Text API must be enabled).',
        sttFailed: true,
        sttCode: 'API_KEY_ERROR',
      }, 422)
    }
    return c.json({
      error: `语音识别失败 — STT failed: ${message.slice(0, 150)}`,
      sttFailed: true,
      sttCode: code,
    }, 422)
  }
  ...
  // ══════════════════════════════════════════════════════════════════
  //
  // OPTIMIZED: sttResult.words are already aligned to natural Chinese
  // word segments by alignSttToReference().  Gemini's only job is to
  // SCORE each pre-grouped word — no character-grouping required.
  // This reduces prompt size ~40% and speeds up response by ~30%.
  //
  // Build word-level and char-level timestamp maps for the
  // normalisation step (in case Gemini re-segments a word).
  ...
  // Pre-grouped word list — Gemini scores these, not groups them
  // Format: "word startMs-endMs" per line (minimal tokens)
  const wordLines = sttResult.words
    .map(w => `${w.word} ${w.startMs}-${w.endMs}`)
    .join('\n')

  const auditPrompt = `You are a Singapore primary-school Mandarin pronunciation diagnostic engine.
The speech recogniser already grouped the words. Your ONLY job: listen to the audio and score each word.
Output ONLY compact JSON. No markdown fences. Start with '{', end with '}'.

Reference passage: ${referenceText}

Words said by student (word startMs-endMs):
${wordLines}
${sttResult.simulated ? '[Timestamps simulated — assess phonetics/tones from audio only]\n' : ''}
For each word above:
- Listen to the audio at the given time range
- Assess tone accuracy and phonetic correctness
- Also add any reference words the student OMITTED (status="omitted")

JSON (omit "p" and "tip" for correct words):
{"words":[{"w":"今天","s":"correct","t":"jīn tiān","startMs":0,"endMs":560},{"w":"清","s":"tone_error","t":"qīng","p":"qing2","startMs":620,"endMs":800,"tip":"Use falling-rising tone (3rd), not rising (2nd)"}],"proficiency":{"pronunciation":85,"tones":80,"fluency":90},"overallComment":"One sentence in English for parents."}

Status: correct | tone_error | wrong | omitted
- pronunciation 0-100 (phoneme accuracy), tones 0-100, fluency 0-100 (pacing)
- Strict: mǎ→mā=tone_error; mǎ→bā=wrong. PURE JSON ONLY.`
  ...
  /**
   * Look up merged timestamp for a word from the STT char index.
   * Used when Gemini outputs a word not in sttMap (e.g., re-segmented).
   */
  function lookupWordTimestamp(word: string): { startMs: number; endMs: number } | null {
    const ts = sttMap.get(word)
    if (ts) return ts
    // Single char
    if (word.length === 1) return sttCharMap.get(word) ?? null
    // Multi-char: find first char, interpolate end
    const first = sttCharMap.get(word[0])
    if (first) return { startMs: first.startMs, endMs: first.startMs + word.length * 280 }
    return null
  }
